...
def plot_clusters_with_new_points(X_scaled, clusters, k, new_data_classifications):
    ...
    #     if status == "Normal":
    #         plt.scatter(point[0], point[1], s=100, color='green', marker='*', edgecolor='k')
    #     else:
    #         plt.scatter(point[0], point[1], s=100, color='red', marker='X', edgecolor='k', label=f'New Point {idx+1} (Abnormal)')
...
# Scale the new data using the same scaler used for the original data
# X_NEWDATA_scaled =  scaler.fit_transform(X_NEWDATA)

# Classify all new data points
new_data_classifications = classify_new_data_points(X_NEWDATA, clusters, thresholds)
